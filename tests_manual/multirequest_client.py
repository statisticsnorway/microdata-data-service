...
def send_req(dataset_name: str, version: str):
    ...
    print(f"process id {process_id} took {stop} with status code {r.status_code}")


if __name__ == '__main__':
    processes = [
        Process(target=send_req, args=[DATASET_VERSION_TUPLES[i][0], DATASET_VERSION_TUPLES[i][1]])
